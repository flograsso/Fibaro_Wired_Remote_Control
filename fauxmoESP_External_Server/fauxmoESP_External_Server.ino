...
ArduinoQueue<movimiento> intQueue(QUEUE_SIZE_ITEMS);
// -----------------------------------------------------------------------------
    ...
    for (int i = 0; i < params; i++) {
      ...
      if (p->isFile()) { //p->isPost() is also true
        ...
        if (p->name() == "ACTION")
          ...
          mov.cortina = p->value().toInt();
        
      }
    }
    intQueue.enqueue(mov); 
...
void loop() {
  ...
  if (!intQueue.isEmpty())
  {
    movimiento mov = intQueue.dequeue();
    Serial.print("Tomo accion de '" + (String)mov.accion + "' sobre cortina: " + (String)mov.cortina);
    delay(5000);
  }
